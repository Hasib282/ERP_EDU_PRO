            ...
            <thead>
                <tr role="row">
                    ...
                    <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1"
                        aria-label="Browser: activate to sort column ascending">Client Name</th>
                    <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1"
            ...
            <tbody>
                @foreach ($inv_client as $key => $item)
                    <tr class="odd">
                        ...
                        <td>
                            @if ($item->status == 1)
                                <a href="{{ route('status', ['table_name' => 'Inv_Client_Info', 'id' => $item->id, 'status' => $item->status]) }}"
                        ...
                        <td style="display: flex;gap:5px;">
                            <button class="btn btn-info btn-sm open-modal editClientModal"
                                data-modal-id="editClientModal" data-id="{{ $item->id }}"><i
                                    class="fas fa-edit"></i>Edit</button>
                            <button class="btn btn-danger btn-sm deleteClient" data-id="{{ $item->id }}"
