...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.1/css/all.min.css" integrity="sha512-MV7K8+y+gLIBoVD59lQIYicR65iaqukzvf/nwasF0nqhPay5w/9lJmVM2hMDcnK1OnMGCdVK+iQrJ7lzPJQd1w==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    <div class="card">
        <div class="card-header">
            <div class="title">
                <h3 class="card-title"><a href="{{ route('add.subCatagory') }}"><button class="add">Add Sub Category</button></a></h3>
            </div>
        </div>
                ...
                <table id="example1" class="table table-bordered table-striped dataTable dtr-inline collapsed"role="grid" aria-describedby="example1_info">
                  ...
                  <tbody>
                    ...
                    <tr class="odd">
                      ...
                      <td>
                        @if($item->status==1)
                            <a href="{{ route('status',['table_name' => 'Inv_Product_Sub_Category', 'id' => $item->id, 'status' => $item->status] ) }}" class="btn btn-success btn-sm"><i class="fa-solid fa-check"></i>Active</a>
                        @else
                            <a href="{{ route('status',['table_name' => 'Inv_Product_Sub_Category', 'id' => $item->id, 'status' => $item->status]) }}" class="btn btn-danger btn-sm">Inactive</a>
                        @endif
                      </td>
                      <td style="display: flex;gap:5px;">
      
                        <a href="{{ route('edit.subCatagory',$item->id) }}" class="btn btn-info btn-sm"><i class="fas fa-edit"></i>Edit</a>
        
                        <form action="{{ route('delete.subCatagory',$item->id) }}" method="post">
                          @csrf
                          @method('Delete')
